...
from skills.legal import verify_clauses   # noqa: E402
...
桩输出 = {
    ...
    "新增条款": "补什么风险：没约定争议解决与送达地址\n"
                "条款文字：「因本合同发生争议，双方应先协商；协商不成的，任何一方可向"
                "＿＿仲裁委员会申请仲裁。双方确认本合同首部所列地址为送达地址。」",
...
任务句 = {"风险清单": "逐条列出对我方的风险", "缺失条款": "对照上面的必查清单",
          ...
          "改法建议": "逐条改写成可以直接替换进合同的条款文字",
          "新增条款": "写一条可以整条贴进合同的完整条款文字", "谈判顺序": "谈判清单"}

# 新增条款 给的是合同里**还不存在**的文字：它没有原文可摘，所以既不该被要求挂锚点，
# 也不该被逐条回查（见 app.py 的 锚点范围）。这条例外本身就是被断言的东西。
不带锚点格式 = {"新增条款"}
...
def _装桩(表=None):
    ...
    def 假问AI(提示词, 判例文字, api_key, 附段落编号=True):
        for 名, 句 in 任务句.items():
            if 句 in 提示词:
                # 判的是**格式要求字符串本身在不在**，不是"提示词里有没有【第N条】" ——
                # 后四维的结论被喂进上文时本来就带方括号，那种断言谁都拦不住（实测过）。
                if 名 == "改法建议":
                    assert verify_clauses.改法格式要求 in 提示词, "改法建议 没带上它自己的格式判据"
                elif 名 in 不带锚点格式:
                    assert verify_clauses.引用格式要求 not in 提示词, f"{名} 不该被要求逐字摘录原文"
                    assert verify_clauses.改法格式要求 not in 提示词, f"{名} 不该被要求挂锚点"
                else:
                    assert verify_clauses.引用格式要求 in 提示词, f"{名} 的提示词没带锚点格式要求"
                return 表.get(名, "")
...
def test_新增条款不进逐条回查而它引的法条照查():
    """这一维是**范围外**，不是**免检**：它给的新文字没有原文可比，
    但它写的法条仍然要过法条库。两半都得断言，否则"排除"会悄悄变成"什么都不查"。"""
    表 = dict(桩输出, 新增条款="补什么风险：解除条件失衡\n"
                              "条款文字：「【第九条】任何一方解除合同应提前30日书面通知。」"
                              "另需核对《劳动合同法》第35条关于协商一致变更的要求")
    _装桩(表)
    数 = _客户端().post("/analyze", json={"name": "运维合同", "text": 合同正文,
                                          "mode": "contract"}).get_json()
    # 合同里根本没有第九条 —— 要是这一维被喂进锚点校验，这条就会被判"疑似编造"
    assert [x["引用条号"] for x in 数["条款校验"]["疑似编造"]] == ["七"], 数["条款校验"]["疑似编造"]
    法名 = {x["法名"] for x in 数["法条对照"]}
    assert "劳动合同法" in 法名, [x["法名"] for x in 数["法条对照"]]
    名 = app_mod.叫法("新增条款")          # 说明用的是卡片上那个名字，不是内部键
    条 = [r for r in 数["风险列表"] if 名 in r["内容"]]
    assert 条 and "不在逐条回查范围内" in 条[0]["内容"], 数["风险列表"]
    assert 条[0]["等级"] == "note", 条[0]


def test_改法与新增同波但都看得到前四维():
    """两波串行是刻意的：它们互不依赖，串行只是白多等一次调用。"""
    抓 = _抓提示词()
    assert "改法建议" in 抓 and "新增条款" in 抓
    for 名 in ("风险清单", "缺失条款"):
        assert f"【{名}】" in 抓["新增条款"], f"新增条款 没看到 {名}"
    assert "【改法建议】" not in 抓["新增条款"], "两维互不依赖：谁都不该等谁"
    assert "【新增条款】" in 抓["谈判顺序"], "谈判顺序 该看到补哪几条"
