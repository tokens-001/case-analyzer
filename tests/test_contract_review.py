...
def test_改法建议的原文摘录照常校验():
    """这一维继承同一套逐字回查 —— 前半段仍是原文摘录。"""
    行 = ('- 【第三条】「合同总价人民币12万元，按季度支付」 改成：'
           '「合同总价人民币12万元。甲方应于每季度末支付该季度款项。」 理由：付款挂验收')
    校 = verify_clauses.verify_clause_anchors(条款正文, 行)
    assert 校["已校验数"] == 1 and 校["疑似编造"] == [], 校


def test_拟稿里的条号不许被当成对现在这份合同的引用():
    """⚠️ 这一维能不能存在就看这条。建议文本常写成"改成：「【第五条】乙方应交付源代码」" ——
    那是**将来那份合同**的结构。不切开的话校验器会拿它去查现在这份的第5条（保密条款），
    查不到就报「疑似编造」，于是每一行正确的改法建议都自带一条红牌。
    对着正确输出亮红牌比漏报更糟：用户学会忽略红色，这套机制就没有可用输出了。

    （原来还有一条同名不同形的弱测试：拟稿只写「…」不带锚点 —— 那种写法正则本来就吃不到，
      把切分整个撤掉它照样绿。所以断言必须落在"拟稿里带锚点"这一形上。）
    """
    行 = ('- 【第三条】「合同总价人民币12万元，按季度支付」 改成：'
           '「【第五条】乙方应交付全部源代码，并删除本合同第三条的表述。」 理由：把保密条改成交付条')
    校 = verify_clauses.verify_clause_anchors(条款正文, 行)
    assert 校["疑似编造"] == [], 校["疑似编造"]
    assert 校["已校验数"] == 1, 校          # 分界之前那段原文摘录照常被核对
    # ⚠️ 光断言"没被指控"是不够的：拟稿里那个【第五条】后面不接「」，
    # 撤掉切分时它不会变成"疑似编造"，而是**悄悄变成一条"无摘录"**（未校验数 +1）。
    # 这两条断言才是真正咬住切分的地方。
    assert 校["无摘录"] == [], 校["无摘录"]
    assert 校["未校验数"] == 0, 校
    assert 校["锚点无法解析"] == [], 校      # 也不许它掉进"看不见就算过"的缝里


def test_改法建议漏掉锚点时仍算未校验():
    """切开的是"拟稿不查"，不是"这一维整体免检"。没挂条号的改法行照样进无锚点条目。"""
    行 = '- 建议把违约金上限改成合同总额的 10%（没给条号）'
    校 = verify_clauses.verify_clause_anchors(条款正文, 行)
    assert 校["无锚点条目"], 校
    assert 校["未校验数"] == 1, 校
...
def test_疑似编造条款进danger而只给条号进warning():
    ...
    assert any(r['等级'] == 'warning' and '没摘原文' in r['内容'] for r in 风险), 风险


def test_同一条号被引两次只列一次():
    """无摘录那份列的是**条号**，不去重会打出"第九、一、九"——读者数成三条，
    而其实是两条引用撞在同一条上。中文条号还得各自带"条"字，否则读起来是半句话。"""
    校验 = _合同校验(无摘录=[{'引用条号': '九'}, {'引用条号': '一'}, {'引用条号': '九'}], 未校验数=3)
    风险 = score_analysis.generate_risk_list({"问题": []}, _合同校验(), None, 条款校验=校验)
    条 = next(r for r in 风险 if r['等级'] == 'warning' and '没摘原文' in r['内容'])
    assert '第九条、第一条' in 条['内容'], 条['内容']
    assert 条['内容'].count('第九条') == 1, 条['内容']
    assert '3 处' in 条['内容'], "计数该保留真实的引用次数，去重只去的是**列出来的条号**"
