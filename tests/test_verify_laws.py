...
def test_动词被吃进法名时不许把库里的法报成库外():
    """实测到的：模型没打书名号，写的是"…明确甲方依据劳动合同法第40条"。
    回退正则只能在字符层面猜边界，把"明确甲方依据"一起当成了法名 ⇒ 文件名对不上 ⇒
    **库里明明有这部法**，却被报成"本工具没收录"。两个后果都实在：
    补库清单被污染（那是给人补法用的），读者还看到一句假覆盖不足。
    认名字的事交给库本身：文件里有什么法，就叫什么法。
    """
    目录 = _假法条库()
    try:
        名表 = verify_laws._库内法名(verify_laws._读取法条库(目录))
        assert "测试法" in 名表, 名表
        assert verify_laws._解析法名("明确甲方依据测试法第5条", 名表) == "测试法"
        出 = verify_laws.classify_law_citations(目录, None, "处理意见：明确甲方依据测试法第5条执行。")
        assert 出["库外"] == [], 出["库外"]
        assert [x["引用"] for x in 出["可验证"]] == ["明确甲方依据测试法第5条"], 出
    finally:
        shutil.rmtree(目录, ignore_errors=True)


def test_书名号里的名字优先于库内名():
    """《旧测试法》不能被认成"测试法"—— 那是两部法，一部现行有效一部已废止。
    长名排前面也救不了这种"短名是长名子串"的情况，所以《》里的名字必须先走。"""
    目录 = _假法条库()
    try:
        名表 = verify_laws._库内法名(verify_laws._读取法条库(目录))
        assert "旧测试法" in 名表 and "测试法" in 名表
        assert verify_laws._解析法名("《旧测试法》第3条", 名表) == "旧测试法"
        # 更要紧的一半：《》里的名字**必须**先走。库里只有"测试法"，而引用的是
        # 《测试法实施细则》第5条 —— 让库内名抢先，它会拿"测试法"去查第5条、查到、
        # 然后对着一部我们根本没有的法亮绿灯。那是这套机制最不能犯的错。
        assert verify_laws._解析法名("《测试法实施细则》第5条", 名表) == "测试法实施细则"
        出 = verify_laws.classify_law_citations(目录, None, "依据《测试法实施细则》第5条办理。")
        assert 出["可验证"] == [], 出["可验证"]
        assert 出["库外"] == ["《测试法实施细则》第5条"], 出
    finally:
        shutil.rmtree(目录, ignore_errors=True)
...
def test_对不上的也算核过不是没核过():
    ...
    assert 可信度["核验"] == {"核到": 1, "对不上": 1, "没法核": 0, "库外": 0,
                          "只给条号": 0, "引用总数": 2}, 可信度["核验"]
    assert 可信度["覆盖度"] == 1.0, 可信度
    assert 可信度["等级"] == "低", 可信度               # 有编造 → 等级照低压


def test_只给条号是没法核的一部分不是另一笔():
    """覆盖度掉下去时必须说清掉在哪：那一维只写条号不摘原文，是"没有可核的东西"，
    不是"核出来有问题"。拆出来的数必须能被 没法核 吸收，两个数各报一遍就是重复计数。"""
    校验 = dict(空校验, 可验证=_可验证(1), 已校验数=1, 未校验数=4)
    条款 = {"可验证": [], "疑似编造": [], "无摘录": [{"引用条号": "九"}, {"引用条号": "一"},
              {"引用条号": "三"}, {"引用条号": "五"}], "未校验数": 4}
    可信度 = score_analysis.compute_trust_score(格式通过, 校验, None, 条款校验=条款)
    核 = 可信度["核验"]
    assert 核["只给条号"] == 4, 核
    assert 核["只给条号"] <= 核["没法核"], 核
    assert 核["引用总数"] == 核["核到"] + 核["对不上"] + 核["没法核"] + 核["库外"], 核
