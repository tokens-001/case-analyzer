...
# ---- 8. 存储 ----
def 存储为JSON(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构, 论证链):
    return _存储为JSON_内部(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构, 论证链)

def _存储为JSON_内部(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构, 论证链):
    文件夹 = 用户数据目录()
    今天 = str(date.today())
    ...
    数据 = {
        ...
        "核心争议": 分析1, "推理链路": 分析2,
        "未回答问题": 分析3, "可平移性": 分析4,
        "反例检索": 反例, "总结": 总结, "结构化摘要": 结构, "论证链检测": 论证链
    }
...
def 分析路由():
    ...
    # 溯源、法条、验证、存储
    溯源 = 溯源对比(判例, 分析1, 分析2, 分析3, 分析4, 反例, 论证链)
    法条对照 = 法条库查询(分析1, 分析2, 分析3, 分析4, 反例, 论证链)
    验证 = 验证分析结果(分析1, 分析2, 分析3, 分析4, 总结)
    存储为JSON(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构, 论证链)
