...
def 分析路由():
    ...
    try:
        if 分析模式 == "case":
            ...
            # 谈判顺序 / 改法建议**必须排在其它四维之后**：它们的提示词写的都是
            # "把上面几类问题……"，而原来 谈判顺序 是和前四维一起并行发出的 ——
            # 那时"上面"还是空的，它只能对着合同原文另拟一套，于是这份清单
            # 和风险清单各说各话（产品里对普通人最有用的一张卡就这么废掉）。
            # 顺序：四维并行 → 改法建议（改的就是这四维点出的问题）→
            #       谈判顺序（指"用哪一份改法文字去提"）→ 总结。
            try:
                结果["改法建议"] = contract_skills.改法建议(判例, 合同类型, api_key, 已有=dict(结果))
            except Exception as e:
                结果["改法建议"] = f"【改法建议失败】{str(e)[:200]}"
            try:
                结果["谈判顺序"] = contract_skills.谈判顺序(判例, 合同类型, api_key, 已有=dict(结果))
            except Exception as e:
                结果["谈判顺序"] = f"【谈判顺序失败】{str(e)[:200]}"
            全部分析 = [结果[k] for k in list(维度) + ["改法建议", "谈判顺序"]]
            try:
    ...
    if 分析模式 == "contract":
        分析结果 = {名: 结果[名] for 名 in ("风险清单", "缺失条款", "失衡条款",
                                        "歧义表述", "改法建议", "谈判顺序")}
        分析结果["总结"] = 总结
