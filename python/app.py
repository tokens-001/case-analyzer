...
# ---- 8. 存储 ----
def 存储为JSON(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构):
    return _存储为JSON_内部(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构)

def _存储为JSON_内部(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构):
    文件夹 = 用户数据目录()
    今天 = str(date.today())
...
def 分析路由():
    ...
    # 溯源、法条、验证、存储
    溯源 = 溯源对比(判例, 分析1, 分析2, 分析3, 分析4, 反例)
    法条对照 = 法条库查询(分析1, 分析2, 分析3, 分析4, 反例)
    验证 = 验证分析结果(分析1, 分析2, 分析3, 分析4, 总结)
    存储为JSON(判例名, 分析1, 分析2, 分析3, 分析4, 反例, 总结, 结构)
