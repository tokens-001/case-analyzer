    ...
    try:
        response = requests.post(
            ...
            json={
                "model": "deepseek-chat",
                "messages": [
                    # 原来这里写死"每个分析维度控制在300字以内" —— 那是**深度天花板**：
# 一条风险要写清"引用了哪句原文、为什么对你不利、依据是哪一条、改成什么"，300 字装不下 12 条。
# 长度是每个任务自己的事，放到提示词里去说，别在共用的系统提示词里一刀切。
{"role": "system", "content": "你是一个法律分析系统。直接输出分析结果，禁止寒暄客套。"
                              "篇幅按任务里写的来，不要自己压缩到看不见理由和依据。"},
